...
# Helper function to preprocess text: lowercase, remove unnecessary punctuation
def preprocess_name(name):
    # name = name.lower()  # Convert to lowercase
    # Keep only alphanumeric characters, spaces, apostrophes, hyphens, and parentheses
...
async def save_data_to_disk(request: Request, db: Session = Depends(get_db)):
    ...
    # Clean and filter song titles
    print("Num total songs: ", len(all_songs))
    valid_songs = clean_and_filter_data([song.title for song in all_songs])
    print("Num valid songs: ", len(valid_songs))

    # Sample 10% of valid songs deterministically
    sampled_songs = sample_data(valid_songs, 0.2)
    print("Num sampled songs: ", len(sampled_songs))

    # Save cleaned and sampled song titles to disk
    ...
    # Save cleaned and filtered artist names
    ...
    # Save cleaned and filtered album titles
